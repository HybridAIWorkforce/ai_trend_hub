...
import { Loader2, Save, Download, Share2, RotateCcw, Copy, Check, Sparkles, Twitter, Linkedin, Facebook, RefreshCw, Edit3, Hash, AlertTriangle, Eye, Code2 } from 'lucide-react';
import { toast } from 'sonner';
import { useRouter } from 'next/navigation';
import { SocialPreviewCards } from './social-preview-cards';
...
export function ArticleEditor({ article: initialArticle }: ArticleEditorProps) {
  ...
  const [socialViewMode, setSocialViewMode] = useState<'edit' | 'preview'>('edit');
    ...
    <div className="max-w-6xl mx-auto space-y-6">
      ...
      <Dialog open={socialPostsDialogOpen} onOpenChange={setSocialPostsDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          ...
          ) : socialPosts.length > 0 && socialViewMode === 'preview' ? (
            /* Styled platform preview cards */
            <div className="mt-2">
              <div className="flex items-center gap-2 mb-4">
                <button
                  onClick={() => setSocialViewMode('edit')}
                  className="flex items-center gap-1 text-xs text-zinc-500 hover:text-white px-3 py-1.5 rounded-lg border border-zinc-700 transition-colors"
                >
                  <Code2 className="w-3 h-3" /> Edit Mode
                </button>
                <button
                  className="flex items-center gap-1 text-xs text-blue-400 px-3 py-1.5 rounded-lg border border-blue-500/50 bg-blue-500/10"
                >
              ...
              <SocialPreviewCards
                posts={socialPosts}
                onUpdatePost={(idx, newContent) => updatePostContent(idx, newContent)}
              />
            </div>
          ) : (
            <div className="space-y-4 mt-2">
              {socialPosts.length > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => setSocialViewMode('preview')}
                    className="flex items-center gap-1 text-xs text-zinc-500 hover:text-white px-3 py-1.5 rounded-lg border border-zinc-700 transition-colors"
                  >
                    <Eye className="w-3 h-3" /> Platform Preview
                  </button>
