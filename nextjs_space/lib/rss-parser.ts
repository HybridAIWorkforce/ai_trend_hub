...
export async function fetchRSSFeed(feedUrl: string, sourceId: string, categoryId: string, sourceName: string) {
  try {
    ...
    for (const item of (feed?.items ?? [])) {
      ...
      if (!existing) {
        const trendScore = calculateTrendScore(publishedAt, tags?.length ?? 0);
...
function calculateTrendScore(publishedAt: Date, tagsCount: number): number {
  const now = new Date();
  const ageInHours = (now.getTime() - publishedAt.getTime()) / (1000 * 60 * 60);
  
  // Decay score based on age (newer items get higher scores)
  const ageScore = Math.max(0, 100 - ageInHours);
  const tagScore = tagsCount * 5;
  
  return Math.min(100, ageScore + tagScore);
}
